...
# Créer des valeurs pour le select_slider
values = []
...
if st.session_state.fission_slider != f"{st.session_state.fission_input:.1e}":
...
selected_value = st.sidebar.select_slider(
    ...
    options=options,
    value=st.session_state['fission_slider'],  # Utilisation de la valeur par défaut correctement formatée
    key="fission_slider"
)

# Convertir la valeur sélectionnée en float pour les calculs
fissions_number = float(selected_value)

# Créer un number_input pour permettre à l'utilisateur d'entrer manuellement la valeur
fissions_number = st.sidebar.number_input(
    "OR enter number of fissions",
    min_value=1.0e+13,
    max_value=1.0e+20,
    value=fissions_number,
    step=None,
    format="%.1e",
    key="fission_input"
)

# Mettre à jour l'état de la session après la création des widgets
if st.session_state.fission_slider != f"{st.session_state.fission_input:.1e}":
    st.session_state.fission_slider = f"{st.session_state.fission_input:.1e}"
    st.session_state.fission_input = float(st.session_state.fission_slider)

# Calculer le facteur de multiplication des doses
dose_multiplier = fissions_number / 1e17

# Afficher la valeur sélectionnée
st.write(f"Selected number of fissions: {fissions_number:.1e}")
...
with tab1:
    ...
    st.write(f"Estimated prompt dose based on total fissions: {fissions_number:.2e}")
    fig1 = go.Figure()
