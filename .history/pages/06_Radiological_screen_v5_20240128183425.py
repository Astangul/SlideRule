...
with tab2:
    ...
    for index, combo in enumerate(compare_data['Filter Combo'].unique()):
        ...
        upper_bound = df_subset['Dose Ratio'] + 2*df_subset['Absolute Combined Uncertainty']
        lower_bound = df_subset['Dose Ratio'] - 2*df_subset['Absolute Combined Uncertainty']

         # Tracé de la ligne principale
        fig2.add_trace(go.Scatter(
            x=df_subset["Distance (m)"], 
            y=df_subset["Dose Ratio"],
            error_y=dict(type='data', array=2*df_subset['Absolute Combined Uncertainty'], visible=True),
            mode='lines+markers',
        ...
        # fig2.add_trace(go.Scatter(x=df_subset["Distance (m)"], 
        #                           y=df_subset["Dose Ratio"],
        #                           error_y=dict(type='data', array=df_subset['Absolute Combined Uncertainty'], visible=True),
        #                           marker_symbol='circle-open-dot', marker_size=8,
        #                           line=dict(dash='dash'),
        #                           mode='lines+markers', 
        #                           name=combo))
