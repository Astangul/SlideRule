...
def scatter_chart(x_data, y_data, err_y_pc):
    ...
    fig.update_xaxes(type="log", showgrid=True, title="Titre de l'axe des abscisses")
    fig.update_xaxes(minor = dict(ticks="inside", ticklen=6, griddash='dot', showgrid=True))
    fig.update_yaxes(type='log', showgrid=True, tickformat='.2e', title="Titre de l'axe des abscisses")
