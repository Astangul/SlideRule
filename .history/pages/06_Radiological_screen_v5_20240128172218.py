...
with tab2:
    ...
    for index, combo in enumerate(compare_data['Filter Combo'].unique()):
        ...
        upper_bound = df_subset['Dose Ratio'] + df_subset['Absolute Combined Uncertainty']
        lower_bound = df_subset['Dose Ratio'] - df_subset['Absolute Combined Uncertainty']
        ...
        fig2.add_trace(go.Scatter(
            ...
            fillcolor=color.replace('#', 'rgba(') + ',0.3)',  # Conversion HEX en RGBA avec transparence
            hoverinfo='none',
            showlegend=False,
            #name='Upper Bound'
        ))
