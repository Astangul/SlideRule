...
with tab2:
    log_x_fig2 = st.checkbox("tototutu", value=True)
    ...
    for index, combo in enumerate(compare_data['Filter Combo'].unique()):
        ...
        legend_group = f"group_{index}"  # Nom unique pour chaque groupe de tracés
        # Définir le template de survol pour formater les valeurs en notation scientifique
        # hovertemplate = '%{y:.2f} ± %{customdata:.2e}'
        hovertemplate='Distance: %{x}<br>Dose Ratio: %{y:.2f}<br>Absolute Combined Uncertainty: %{customdata:.2e}<extra></extra>'

         # Tracé de la ligne principale
        fig2.add_trace(go.Scatter(
            x=df_subset["Distance (m)"], 
            y=df_subset["Dose Ratio"],
            customdata=df_subset['Absolute Combined Uncertainty'],  #
