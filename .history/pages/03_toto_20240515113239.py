...
def dose_ratio_bar_chart(compare_data, ref_data, colors):
    ...
    for index, (key, group) in enumerate(compare_data.groupby('unique_key')):
        ...
        unique_distances = sorted(group['Distance (m)'].unique())
        distance_labels = [str(dist) for dist in unique_distances]

        # Créer des barres pour chaque distance unique
        for distance in unique_distances:
            subset = group[group['Distance (m)'] == distance]
            if not subset.empty:
                fig2.add_trace(go.Bar(
                    x=[str(distance)],
                    y=subset['Dose Ratio'],
                    customdata=subset['Absolute Combined Uncertainty'],
                    error_y=dict(type='data', array=subset['Absolute Combined Uncertainty'], visible=True),
                    name=key,
                    marker_color=color,
                    hovertemplate=hovertemplate
                ))
    ...
    # Toggle for logarithmic scale on X-axis
    log_x_fig2 = st.toggle("X-axis log scale", value=True, key="log_x_fig2_bar")
