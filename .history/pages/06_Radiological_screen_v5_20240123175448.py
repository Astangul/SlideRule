...
with tab2:
    ...
    xaxis={
        'type': 'log',
        'showgrid': True,
        'title': "Distance (m) [Log10]"  # Titre de l'axe X
    },
    yaxis={
        'showgrid': True,
        'title': "Ratio des Doses",  # Titre de l'axe Y
        'range': [0.5, 1.5]  # Limites centrées autour de 1 pour l'axe Y
    })
    fig2.update_yaxes(minor = dict(ticks="inside", ticklen=6, griddash='dot', showgrid=True))
