...
def calculate_differences(group):
    # Vous pouvez calculer ici les différences entre les codes, par exemple :
    if 'MCNP 6.1' in group.columns and 'SCALE 6.2.2' in group.columns:
        group['Ecart_MCNP_SCALE'] = (group['MCNP 6.1'] - group['SCALE 6.2.2']).abs() / group['MCNP 6.1']
    # Ajoutez des calculs similaires pour les autres paires de codes si nécessaire
    return group

detailed_data = data.groupby(columns_to_group_by).apply(calculate_differences)
st.dataframe(detailed_data)
# ecarts_importants = detailed_data[detailed_data['Ecart_MCNP_SCALE'] > seuil_ecart]

# # Afficher les résultats
# with tab6:  # Utilisation de l'onglet approprié dans Streamlit
#     if not ecarts_importants.empty:
#         st.write("Écarts significatifs entre les doses calculées par les différents codes :")
#         st.dataframe(ecarts_importants)
#     else:
#         st.write("Aucun écart significatif trouvé.")
