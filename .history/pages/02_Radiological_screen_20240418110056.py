...
#                 if ecart_relatif > 0.1:
#                     # Création du dictionnaire pour le nouveau DataFrame
#                     new_row_data = {
#                         'Groupe': [str(name)],
#                         'Code1': [code1],
#                         'Code2': [code2],
...
#                     # Ajouter les détails du groupe
#                     if isinstance(name, tuple):  # Si le groupe est par plusieurs colonnes
#                         for col, val in zip(grouping_columns, name):
#                             new_row_data[col] = [val]
#                     else:  # Si le groupe est par une seule colonne
#                         new_row_data[grouping_columns[0]] = [name]
                    
#                     new_row = pd.DataFrame(new_row_data)
#                     significant_differences = pd.concat([significant_differences, new_row], ignore_index=True)
...
#                 dose_code2 = local_group[local_group['Code'] == code2]['Dose (Gy)'].iloc[0]
#                 ecart_relatif = abs(dose_code1 - dose_code2) / max(dose_code1, dose_code2) 
#                 # uncertainty_code1 = local_group[local_group['Code'] == code1]['1s uncertainty'].iloc[0] * dose_code1 # Absolute uncertainty
#                 #  uncertainty_code2 = local_group[local_group['Code'] == code2]['1s uncertainty'].iloc[0] * dose_code2
#                 #  sigma_comb = np.sqrt(uncertainty_code1**2 + uncertainty_code2**2)

#     # for name, local_group in grouped_data:
#     #     codes = local_group['Code'].unique()
#     #     for i, code1 in enumerate(codes):
#     #         for code2 in codes[i + 1:]:
#     #             doses = local_group[local_group['Code'].isin([code1, code2])]
#     #             if doses.shape[0] < 2:
#     #                 continue
#     #             dose_code1, dose_code2 = doses['Dose (Gy)'].values
#     #             uncertainty_code1, uncertainty_code2 = doses['1s uncertainty'].values * doses['Dose (Gy)'].values
#     #             ecart_relatif = abs(dose_code1 - dose_code2) / max(dose_code1, dose_code2)
