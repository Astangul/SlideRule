...
# Continuation du calcul des écarts pour chaque groupe
for name, group in grouped_data:
    codes = group['Code'].unique()  # S'assurer d'utiliser les codes uniques dans ce groupe
    for i, code1 in enumerate(codes):
        for code2 in codes[i + 1:]:
            # Sélectionner les doses et les incertitudes pour chaque code
            dose_code1 = group[group['Code'] == code1]['Dose (Gy)'].iloc[0]
            uncertainty_code1 = group[group['Code'] == code1]['1s uncertainty'].iloc[0]
            dose_code2 = group[group['Code'] == code2]['Dose (Gy)'].iloc[0]
            uncertainty_code2 = group[group['Code'] == code2]['1s uncertainty'].iloc[0]
            ...
            if pd.notnull(dose_code1) and pd.notnull(dose_code2) and dose_code1 != 0 and dose_code2 != 0:
                ...
                if ecart_relatif > seuil_ecart:
                    ecarts_significatifs = ecarts_significatifs.append({
                        ...
                        'Code1': code1,
                        'Code2': code2,
                        'Dose_Code1': f"{dose_code1:.3e}",
                        'Dose_Code2': f"{dose_code2:.3e}",
                        'Uncertainty_Code1': f"{uncertainty_code1:.2%}",
                        'Uncertainty_Code2': f"{uncertainty_code2:.2%}",
                        'Ecart_Relatif': ecart_relatif
                    }, ignore_index=True)

# Maintenant, vous pouvez afficher le DataFrame dans Streamlit
with tab6:
    if not ecarts_significatifs.empty:
        st.write("Écarts significatifs entre les doses calculées par les différents codes et leurs incertitudes associées :")
