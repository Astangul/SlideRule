...
tab4, tab5, tab6 = st.tabs(["❔ Missing values", "❗ High uncertainty (1σ>10%)", "🚨 Inter-codes discrepancies"])

with tab4:
    empty_values_df = data[data.isnull().any(axis=1)]
    st.dataframe(empty_values_df)
...
with tab5:
    ...
    st.dataframe(high_uncertainty_df)
...
grouped_data = data.groupby(columns_to_group_by)

# Seuil d'écart significatif, par exemple 5%
seuil_ecart = 0.05

# Initialiser un DataFrame pour collecter les écarts significatifs
ecarts_significatifs = pd.DataFrame()

# Calculer les écarts pour chaque groupe
for name, group in grouped_data:
    codes = group['Code'].unique()  # S'assurer d'utiliser les codes uniques dans ce groupe
    for i, code1 in enumerate(codes):
        for code2 in codes[i + 1:]:
            # Sélectionner les doses pour chaque code
            dose_code1 = group[group['Code'] == code1]['Dose (Gy)'].iloc[0]
            dose_code2 = group[group['Code'] == code2]['Dose (Gy)'].iloc[0]

            # Vérifier si les deux codes ont des doses à comparer et que les doses ne sont ni nulles ni NaN
            if pd.notnull(dose_code1) and pd.notnull(dose_code2) and dose_code1 != 0 and dose_code2 != 0:
                ecart_relatif = abs(dose_code1 - dose_code2) / max(dose_code1, dose_code2)

                # Si l'écart dépasse le seuil, on enregistre le résultat
                if ecart_relatif > seuil_ecart:
                    ecarts_significatifs = ecarts_significatifs.append({
                        'Groupe': str(name),
                        'Code1': code1,
                        'Code2': code2,
                        'Dose_Code1': dose_code1,
                        'Dose_Code2': dose_code2,
                        'Ecart_Relatif': ecart_relatif
                    }, ignore_index=True)


# Conversion des types après le remplissage pour éviter les problèmes de type avec Arrow
ecarts_significatifs = ecarts_significatifs.convert_dtypes()

# Pour l'affichage, convertir les valeurs numériques en chaînes avec un format scientifique
ecarts_significatifs['Dose_Code1'] = ecarts_significatifs['Dose_Code1'].apply(lambda x: f"{x:.2e}")
ecarts_significatifs['Dose_Code2'] = ecarts_significatifs['Dose_Code2'].apply(lambda x: f"{x:.2e}")

# Maintenant, vous pouvez essayer d'afficher le DataFrame dans Streamlit
with tab6:
    if not ecarts_significatifs.empty:
        st.write("Écarts significatifs entre les doses calculées par les différents codes :")
        st.dataframe(ecarts_significatifs)
    else:
        st.write("Aucun écart significatif trouvé entre les codes.")