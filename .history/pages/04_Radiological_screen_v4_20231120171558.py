...
with st.expander("1/ Select Reference Case"):
    ...
    with tab1:
        ...
        SR_reference_filters, df_SR_reference = dataframe_selectbox_filters(df_SR_original)
    
    with tab2:
        # Contenu pour l'onglet "Data"
        st.dataframe(df_SR_reference)
    
    with tab3:
        # Contenu pour l'onglet "Graph"
        scatter_chart(df_SR_reference['Distance (m)'], df_SR_reference['Dose (Gy)'], 2*df_SR_reference['1s uncertainty'])
...
st.write("DataFrame filtré (v1) :", filtered_SR_df) # = #st.dataframe(filtered_SR_df)

filtered_df = df_SR_original  # Initialise le DataFrame filtré avec le DataFrame d'origine

for column, values in SR_filters.items():
...
# filtered_df contient maintenant le DataFrame original filtré en fonction des filtres stockés dans SR_filters
st.write("DataFrame filtré (v2):", filtered_SR_df)