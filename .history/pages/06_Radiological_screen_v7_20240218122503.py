...
with st.expander("Click to expand/collapse", expanded=False):
    ...
    with col1:
        ...
        compare_data, compare_filters = df_multiselect_filters(data, default_columns=['Fissile', 'Case', 'Code', 'Particle', 'Screen', 'Thickness (cm)'], key="compare_series")
    with col2:
        # Utiliser df_selectbox_filters pour sélectionner le cas de référence
        st.subheader("Reference case")
        ref_data, ref_filters = df_selectbox_filters(data, default_columns=['Fissile', 'Case', 'Code', 'Particle', 'Screen', 'Thickness (cm)'], key="reference_case")
...
with tab1:
    ...
    if ref_filters:
        # Utiliser les valeurs de ref_filters pour construire le libellé
        ref_label = "_".join([str(value) for value in ref_filters.values()]) + " (reference)"
    else:
        # Gérer le cas où ref_filters n'est pas défini ou vide
        ref_label = "Reference case not selected"
