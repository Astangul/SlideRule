...
def dose_ratio_scatter_plot_2(compare_data, ref_data, colors):
...
def dose_ratio_bar_chart_2(compare_data, ref_data, colors):
    title = "Dose Ratio"
    ...
    # Group the data by 'unique_key' and sort the groups by the specified column
    grouped_data = list(compare_data.groupby('unique_key'))
    grouped_data.sort(key=lambda x: x[1]['Thickness (cm)'].iloc[0])  # Sort by the first occurrence in each group

    # Loop through each group by 'unique_key'
    # for index, (key, group) in enumerate(compare_data.groupby('unique_key')):
    for index, (key, group) in enumerate(grouped_data):
        color = colors[index % len(colors)]
        rgba_color = hex_to_rgba(color, alpha=1.0)
        ...
        fig2.add_trace(go.Bar(
            ...
            error_y=dict(
                ...
                visible=True,
                color=complementary_color,  # Same color as the bars
...
def dose_ratio_bar_chart(compare_data, ref_data, colors):
    ...
    for index, (key, group) in enumerate(grouped_data):
        color = colors[index % len(colors)]
        rgba_color = hex_to_rgba(color, alpha=0.1)  # Adding transparency to the bar colors
