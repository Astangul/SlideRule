...
with st.expander("↳", expanded=True):
    ...
    with col1:
        st.subheader("Séries à Comparer (multiselect)")
...
with tab1:
    ...
    fig1.add_trace(go.Scatter(x=ref_data["Distance (m)"],
                              ...
                              mode='lines+markers', 
                              name=ref_label,
                              error_y=dict(type='data', array=10*ref_data["Absolute Uncertainty"], visible=True)
                              ))
    ...
    for combo in compare_data['Filter Combo'].unique():
        ...
        fig1.add_trace(go.Scatter(x=df_subset["Distance (m)"], 
                                  ...
                                  line=dict(dash='dash'),
                                  name=combo,
                                  error_y=dict(type='data', array=10*df_subset["Absolute Uncertainty"], visible=True)
                                  ))
