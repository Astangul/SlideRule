...
# def generate_dose_ratio_plot(compare_data, ref_data, colors):
#     # Paramètres internes de la fonction
...
def generate_dose_ratio_plot(compare_data, ref_data, colors, log_scale=False):
    # Paramètres internes de la fonction
    title = "Dose Ratio"
    ...
    # Set X-axis scale based on the toggle
    if log_scale:
        fig2.update_xaxes(type='log', title="Distance (m) [Log10]")
    else:
        fig2.update_xaxes(type='linear', title="Distance (m)")
    ...
    # Render the chart with the updated axis
    st.plotly_chart(fig2, use_container_width=True)

with tab2:
        # Initial rendering of the plot with a default linear scale
    generate_dose_ratio_plot(compare_data, ref_data, colors, log_scale=False)
    
    # Toggle for logarithmic scale on X-axis placed below the initial plot
    log_x_fig2 = st.toggle("X-axis log scale", value=True, key="log_x_fig2")
    
    # Re-render the plot based on the toggle value
    generate_dose_ratio_plot(compare_data, ref_data, colors, log_scale=log_x_fig2
@st.cache_data
