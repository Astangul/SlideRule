...
ecarts_significatifs['Groupe'] = ecarts_significatifs['Groupe'].astype(str)  # Conversion en string si c'est un identifiant ou une catégorie
ecarts_significatifs['Ecart_Relatif'] = ecarts_significatifs['Ecart_Relatif'].astype(float)

# Calculer les écarts pour chaque groupe
for name, group in grouped_data:
    for i in range(len(codes_uniques)):
        for j in range(i + 1, len(codes_uniques)):
            code1 = codes_uniques[i]
            code2 = codes_uniques[j]
            dose_code1 = group[group['Code'] == code1]['Dose (Gy)']
            dose_code2 = group[group['Code'] == code2]['Dose (Gy)']
            # Vérifier si les deux codes ont des doses à comparer
            if not dose_code1.empty and not dose_code2.empty:
                ecart_relatif = abs(dose_code1.iloc[0] - dose_code2.iloc[0]) / max(dose_code1.iloc[0], dose_code2.iloc[0])
                ...
                if ecart_relatif > seuil_ecart:
                    ecarts_significatifs = ecarts_significatifs.append({
                        ...
                        'Code1': code1,
                        'Code2': code2,
                        'Dose_Code1': dose_code1.iloc[0],
                        'Dose_Code2': dose_code2.iloc[0],
