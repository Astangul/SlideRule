...
# # Slider en notation scientifique pour le nombre de fissions
# fissions = st.slider("Sélectionnez le nombre de fissions :", 
#                      min_value=1e16, max_value=1e20, 
#                      value=1e17, step=1e15, 
#                      format="%.1e")

# st.write(f"Nombre de fissions sélectionné : {fissions} fissions")
...
# slider_value = st.slider("Nombre de fissions (x1E15):", min_value=1, max_value=2000, value=1100, step=1, format='%.1e')
...
# #Méthode 4 
# # Définir les limites min et max de l'exposant pour la base 10
# min_exponent, max_exponent = 16, 20

# # Créer un slider pour sélectionner l'exposant
# exponent = st.slider('Sélectionnez l\'exposant pour le nombre de fissions:',
#                      min_value=min_exponent, max_value=max_exponent, 
#                      value=17, step=1, 
#                      format="1e%d")

# # Convertir l'exposant sélectionné en valeur réelle
# fissions = np.power(10, exponent)

# st.write(f"Nombre de fissions sélectionné: {fissions:.1e}")

#Methode 5
# Générer une liste de valeurs en échelle logarithmique
# Générer une liste de valeurs en échelle logarithmique avec des pas de 0.1
values = []
for exponent in range(16, 21):  # Pour 1.0E16 à 1.0E20
    base = 10 ** exponent
...
selected_value = st.select_slider(
    'Sélectionnez le nombre de fissions :',
    options=options,
