...
def filter_dataframe_multiselect(df: pd.DataFrame) -> pd.DataFrame:
    ...
    with modification_container:
        to_filter_columns = st.multiselect("Filter dataframe on", df.columns, list(df[column].unique()))
...
def filter_dataframe_test(df: pd.DataFrame) -> (pd.DataFrame, dict):
    """
    Adds a UI on top of a dataframe to let viewers filter columns
...
tab1, tab2 = st.tabs(["Reference Case", "Filtered Data"])
with tab1:
    df_test = filter_dataframe_multiselect(df_SR_original)
with tab2:
