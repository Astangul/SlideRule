...
for name, group in grouped_data:
    ...
    for i, code1 in enumerate(codes):
        for code2 in codes[i + 1:]:
            ...
            # Calculer l'écart-type combiné (σ comb)
            sigma_comb = (uncertainty_code1**2 + uncertainty_code2**2)**0.5

            # Calculer la différence absolue entre les doses
            difference = abs(dose_code1 - dose_code2)

            # Comparer la différence absolue à l'écart-type combiné
            if difference > 3*sigma_comb:
                # Si la différence est plus grande que σ comb, enregistrer le résultat
                ecarts_significatifs = ecarts_significatifs.append({
                    ...
                    'Sigma_Comb': f"{sigma_comb:.2e}",
                    'Difference': f"{difference:.2e}"
                }, ignore_index=True)

# Affichage des résultats dans Streamlit
with tab6:
    if not ecarts_significatifs.empty:
        st.write("Écarts significatifs entre les doses calculées par les différents codes, comparés à l'écart-type combiné :")
