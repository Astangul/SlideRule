...
def filter_df_multiselect(df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds a UI on top of a dataframe to let viewers filter columns
    ...
    # Create tabs to display the filtered DataFrame
    tab1 = st.tabs(["Filtered DataFrame"])
    with tab1:
        # Modification container for filtering
        modification_container = st.container()
        with modification_container:
            to_filter_columns = st.multiselect("Filter dataframe on", df.columns)
            filtered_df = df.copy()
            for column in to_filter_columns:
                ...
                if is_categorical_dtype(df[column]) or df[column].nunique() < 10:
                    ...
                    if user_text_input:
                        filtered_df = filtered_df[filtered_df[column].str.contains(user_text_input)]
