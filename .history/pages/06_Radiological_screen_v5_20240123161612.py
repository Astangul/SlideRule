...
with col2:
    ...
    ref_case = st.selectbox('Cas:', options=list(data['Case'].unique()))
...
tab1, tab2, tab3 = st.tabs(["📈 Graphe des Doses", "🆚 Graphe des Ratios", "🔢 Tableau des Données"])
...
with tab1:
    ...
    fig1.add_trace(go.Scatter(x=ref_data["Distance (m)"],
                              ...
                              mode='lines+markers', 
                              name=ref_label,
                              error_y=dict(type='data', array=ref_data["Absolute Uncertainty"], visible=True)
                              ))
...
# Mise à jour conditionnelle des axes
    ...
    if log_y:
        fig1.update_yaxes(type='log', title="Dose (Gy)[Log10]", tickformat='.2e')
    else:
        fig1.update_yaxes(type='linear', title="Dose (Gy)",  tickformat='.2e')
