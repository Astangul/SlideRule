...
def filter_df_selectbox(df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds a UI on top of a dataframe to let viewers filter columns
...
def filter_dataframe_test(df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds a UI on top of a dataframe to let viewers filter columns
    ...
    with modification_container:
        to_filter_columns = st.multiselect("Filter dataframe on", df_filtered.columns)
        for column in to_filter_columns:
            left, right = st.columns((1, 20))
            left.write("↳")
            # Treat columns with < 10 unique values as categorical
            if is_categorical_dtype(df_filtered[column]) or df_filtered[column].nunique() < 10:
                ...
                user_date_input = right.date_input(
                    f"Values for {column}",
                    value=(
                        df_filtered[column].min(),
                        df_filtered[column].max(),
                    ),
                )
                if len(user_date_input) == 2:
                    user_date_input = tuple(map(pd.to_datetime, user_date_input))
                ...
                if user_text_input:
                    df_filtered = df_filtered[df_filtered[column].str.contains(user_text_input)]

    # Display the filtered DataFrame in a new tab
    with st.container():
...
with st.expander("Select reference case"):
    st.dataframe(filter_df_multiselect(df_SR_original))