...
def dataframe_selectbox_filters(df: pd.DataFrame, default_columns: list = None, key: str = "default") -> (pd.DataFrame, dict):
    """
    Adds a UI on top of a dataframe to let viewers filter columns with an option to specify default columns to display
    ...
    if not modify:
        return df, {}  # Return the original dataframe and an empty filter if no modifications are applied
